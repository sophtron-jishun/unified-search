...
function populateEntryProperties (entry, s, sourceSchema, sourceId) {
  const routing_number = s[sourceSchema.routing_number]
...
function processProvider (source, mapping, source_provider, mapped_provider, sourceSchema, mappingSchema) {
  ...
  for (let i = 1; i < source.length; i++) {
    const s = source[i]
    const sourceId = s[sourceSchema.id]
    // let log = s[sourceSchema['name']].toLowerCase() === 'pnc bank' && (mapped_provider === 'mx' || source_provider === 'mx')

    // find the mapping if exists
    const mappedId = mapped_provider
      ? mapping.find(item =>
        item[mappingSchema[source_provider]] === sourceId)?.[mappingSchema[mapped_provider]]
      : null
    ...
    let matched = false
    for (const en of db.current.foreignIndexes.get(source_provider)?.get(sourceId) || []) {
    ...
    if (mappedId) {
      for (const en of db.current.foreignIndexes.get(mapped_provider)?.get(mappedId) || []) {
    ...
    const uid = mapped_provider ? sourceId : `${source_provider}_${sourceId}`
    const entry = {
      id: uid,
      foreignKeys: {
...
(async function entry () {
  ...
  for (const [key, item] of db.current.mainIndex) {
    if (item.name) {
