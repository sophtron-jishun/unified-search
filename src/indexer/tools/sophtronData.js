...
function filterInstitution (item) {
  const name = item.InstitutionName.trim().toLowerCase()
  return !name.endsWith('v2') &&
    ...
    !blackList.includes(item.InstitutionID)
}
...
sophtronClient.batchGetInstitutions().then(all => {
  utils.arrayToCsvFile(
    ...
    (item) => {
      const routing_number = `${item.InstitutionDetail.RoutingNumber}/${item.InstitutionDetail.MultipleRoutingNumbers}`.trim()
      const arr = routing_number.split('/').filter((r, i, a) => a.indexOf(r) === i)
